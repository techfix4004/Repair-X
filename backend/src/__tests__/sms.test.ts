...
describe('SMS Management API Tests', () => {
    ...
    app.post('/api/v1/sms/send', async (request, reply: any) => {
      ...
      return reply.send({
        ...
        data: {
          ...
          recipient: (smsData as any)._recipient,
          message: (smsData as any)._message,
          status: 'sent',
          credits: 1,
    ...
    app.post('/api/v1/sms/templates', async (request, reply: any) => {
      ...
      return reply.code(201).send({
        ...
        data: {
          ...
          name: (templateData as any)._name,
          content: (templateData as any)._content,
          variables: (templateData as any)._variables || [],
          category: (templateData as any)._category || 'general',
          active: true
        },
  ...
  test('POST /api/v1/sms/send - should send SMS successfully', async () => {
    ...
    expect(body.data?.recipient).toBe(smsData._recipient);
  ...
  test('POST /api/v1/sms/templates - should create SMS template', async () => {
    ...
    const body = JSON.parse(response.payload);
    expect(body.success).toBe(true);
    expect(body.data?.name).toBe(templateData._name);
    expect(body.data?.variables).toEqual(templateData._variables);
